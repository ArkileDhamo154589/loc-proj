...
// On Vercel, work passed to waitUntil keeps running after the response is sent (what @vercel/functions uses).
// Elsewhere (local server, tests) the caller awaits the task instead.
...
export async function handleBookingRequest(input, rawEnv = process.env, log = console) {
  ...
  if (env.SHEETS_WEBHOOK_URL) {
    ...
    try {
      // Save the row first and answer the visitor; emails go in a second call so a slow
      // Gmail send can never push the function past the platform time limit.
      const json = await postToSheet({ ...sheet, row: request, capacity, mails: [] });
  ...
  let mailed = false;
  if (mailVia === 'sheet' && sinks.includes('sheet')) {
    const task = postToSheet({
      url: env.SHEETS_WEBHOOK_URL,
      token: env.SHEETS_TOKEN || '',
      action: 'mail',
      mails: mails.map((m) => ({ ...m, fromName: 'Meltemi Rentals' })),
    })
      .then((json) => {
        if (!json.ok || json.mailErrors?.length) log.error('[booking] sheet mail errors', request.ref, json.error || json.mailErrors.join(' | '));
      })
      .catch((err) => log.error('[booking] sheet mail request failed', request.ref, err.message));
    if (!runInBackground(task)) await task;
    mailed = true;
  }
